...
#import "ORKatrinMchController.h"
...
@interface ORKatrinMchController (private)
...
@implementation ORKatrinMchController

#pragma mark •••Initialization
-(id)init
{
    self = [super initWithWindowNibName:@"KatrinMch"];
    return self;
}
...
- (void) awakeFromNib
{
	...
	[[self window] setTitle:@"Katrin MCH"];
...
- (void) registerNotificationObservers
{
	...
	[notifyCenter addObserver : self
                     selector : @selector(hwVersionChanged:)
                         name : ORKatrinMchModelHwVersionChanged
                       object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(statusRegChanged:)
                         name : ORKatrinMchModelStatusRegChanged
						object: model];
	
	[notifyCenter addObserver : self
                     selector : @selector(controlRegChanged:)
                         name : ORKatrinMchModelControlRegChanged
                       object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(selectedRegIndexChanged:)
						 name : ORKatrinMchSelectedRegIndexChanged
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(writeValueChanged:)
						 name : ORKatrinMchWriteValueChanged
					   object : model];
		
    [notifyCenter addObserver : self
                     selector : @selector(pulserAmpChanged:)
                         name : ORKatrinMchPulserAmpChanged
                       object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(pulserDelayChanged:)
                         name : ORKatrinMchPulserDelayChanged
                       object : model];
			
    [notifyCenter addObserver : self
                     selector : @selector(interruptMaskChanged:)
                         name : ORKatrinMchModelInterruptMaskChanged
						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(pollRateChanged:)
                         name : ORKatrinMchPollTimeChanged
                       object : model];
		
    [notifyCenter addObserver : self
                     selector : @selector(patternFilePathChanged:)
                         name : ORKatrinMchModelPatternFilePathChanged
						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(secondsSetChanged:)
                         name : ORKatrinMchModelSecondsSetChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(deadTimeChanged:)
                         name : ORKatrinMchModelDeadTimeChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(vetoTimeChanged:)
                         name : ORKatrinMchModelVetoTimeChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(runTimeChanged:)
                         name : ORKatrinMchModelRunTimeChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(clockTimeChanged:)
                         name : ORKatrinMchModelClockTimeChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(countersEnabledChanged:)
                         name : ORKatrinMchModelCountersEnabledChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(sltScriptArgumentsChanged:)
                         name : ORKatrinMchModelSltScriptArgumentsChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(secondsSetInitWithHostChanged:)
                         name : ORKatrinMchModelSecondsSetInitWithHostChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(secondsSetSendToFLTsChanged:)
                         name : ORKatrinMchModelSecondsSetSendToFLTsChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(pixelBusEnableRegChanged:)
                         name : ORKatrinMchModelPixelBusEnableRegChanged
						object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(lostEventsChanged:)
                         name : ORKatrinMchModelLostEventsChanged
                        object: model];

    [notifyCenter addObserver : self
                     selector : @selector(lostFltEventsChanged:)
                         name : ORKatrinMchModelLostFltEventsChanged
                        object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(lostFltEventsTrChanged:)
                         name : ORKatrinMchModelLostFltEventsTrChanged
                        object: model];

    [notifyCenter addObserver : self
                     selector : @selector(minimumDecodingChanged:)
                         name : ORKatrinMchModelMinimizeDecodingChanged
                        object: model];
}
...
- (void) settingsLockChanged:(NSNotification*)aNotification
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORKatrinMchSettingsLock];
    BOOL locked = [gSecurity isLocked:ORKatrinMchSettingsLock];
...
{
    [gSecurity tryToSetLock:ORKatrinMchSettingsLock to:[sender intValue] forWindow:[self window]];
}
...
@implementation ORKatrinMchController (private)
#if !defined(MAC_OS_X_VERSION_10_10) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_10 // 10.10-specific
- (void) calibrationSheetDidEnd:(id)sheet returnCode:(int)returnCode contextInfo:(NSDictionary*)userInfo
{
    if(returnCode == NSAlertAlternateReturn){
