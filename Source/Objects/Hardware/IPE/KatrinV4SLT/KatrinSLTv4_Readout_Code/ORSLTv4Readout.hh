...
class ORSLTv4Readout : public ORVCard
{
  public:
   ...
   private:
    bool firstTime;
    uint64_t nNoData; //< Number of loops where no data was read
    
    struct timeval t0;
    struct timeval t1;
    uint32_t numWordsToRead; //< Number of word read in the last cycle
    uint64_t nWords; //< Number of raw bytes recorded
    uint64_t nLoops; //< Number of calls to the readout function
    uint64_t nReadout; //< Number of calls with readout of data
    uint64_t nReducedSize; //< Number of calls and readout of less than maximum block size
    uint64_t nWaitingForReadout; //< Number of loops waiting for data
    uint64_t nInhibit; //< Number of loops with inhibit active
    uint64_t nNoReadout; //<  Number of loops without readout
    uint64_t maxLoopsPerSec; //< Speed of the readout loop; used for analysis
