...
NSString* ORIpeMtcaCrateModelUnlockedStopButtonChanged = @"ORIpeMtcaCrateModelUnlockedStopButtonChanged";
NSString* ORIpeMtcaCrateModelSnmpPowerSupplyIPChanged = @"ORIpeMtcaCrateModelSnmpPowerSupplyIPChanged";
NSString* ORIpeMtcaCrateConnectedChanged = @"ORIpeMtcaCrateConnectedChanged";

@implementation ORIpeMtcaCrateModel
...
- (void) setUpImage
{
    ...
    //---------------------------------------------------------------------------------------------------
    NSImage* aCachedImage = [NSImage imageNamed:@"IpeMtcaCrate"];
    ...
    if(powerOff){
        NSAttributedString* s = [[[NSAttributedString alloc] initWithString:@"No Pwr"
                attributes:[NSDictionary dictionaryWithObjectsAndKeys: [NSColor redColor],NSForegroundColorAttributeName, [NSFont fontWithName:@"Geneva" size:10],NSFontAttributeName, nil]] autorelease];
        [s drawAtPoint:NSMakePoint(25,5)];
    }
    
    if([[self orcaObjects] count]){
        NSAffineTransform* transform = [NSAffineTransform transform];
        [transform translateXBy:35 yBy:40];
        [transform scaleXBy:0.70 yBy:0.46];
        [transform concat];
...
{
    [self linkToController:@"ORIpeMtcaCrateController"];
}
...
- (void) setUnlockedStopButton:(int)aUnlockedStopButton
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORIpeMtcaCrateModelUnlockedStopButtonChanged object:self];
}
...
- (void) setSnmpPowerSupplyIP:(NSString*)aSnmpPowerSupplyIP
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORIpeMtcaCrateModelSnmpPowerSupplyIPChanged object:self];
}
- (void) setIsConnected:(BOOL)aState
{
	isConnected = aState;
    [[NSNotificationCenter defaultCenter] postNotificationName:ORIpeMtcaCrateConnectedChanged object:self];
}
...
- (NSString*) nameForSlot:(int)aSlot {
    return [NSString stringWithFormat:@"Station %d",aSlot+1];
}

- (NSRange) legalSlotsForObj:(id)anObj
{
	if( [anObj isKindOfClass:NSClassFromString(@"ORIpeMtcaMasterModel")]
       || [anObj isKindOfClass:NSClassFromString(@"ORKatrinMchModel")] ){
		return NSMakeRange(12,1);
	}
	else {
