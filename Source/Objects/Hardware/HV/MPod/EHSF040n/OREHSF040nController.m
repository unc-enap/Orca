...
#import "OREHSF040nController.h"
#import "OREHSF040nModel.h"
#import "ORDetectorRamper.h"

@implementation OREHSF040nController

-(id)init
{
    self = [super initWithWindowNibName:@"EHSF040n"];
    return self;
}
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(tripTimeChanged:)
                         name : OREHSF040nModelTripTimeChanged
						object: model];


    [notifyCenter addObserver : self
                     selector : @selector(currentTripBehaviorChanged:)
                         name : OREHSF040nModelCurrentTripBehaviorChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(outputFailureBehaviorChanged:)
                         name : OREHSF040nModelOutputFailureBehaviorChanged
						object: model];
...
{
	if(aTableView == ramperTableView)	return 16;
...
{
	if(aTableView == hvTableView){
		NSParameterAssert(rowIndex >= 0 && rowIndex < 16);
