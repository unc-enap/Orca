	...
	if([self count]){
		...
		while(aCard = [e nextObject]){
			if([aCard isKindOfClass:[ORVmeIOCard class]]){
				...
				NSString* s = [NSString stringWithFormat:@"%4d %-20s 0x%08lx - 0x%08lx",[aCard slot],[[aCard className] cStringUsingEncoding:NSASCIIStringEncoding],aRange.location,NSMaxRange(aRange)];
				NSLogFont([NSFont fontWithName:@"Monaco" size:12], @"%@\n",s);
			}
		}
		NSLogFont([NSFont fontWithName:@"Monaco" size:12], @"--------------------------------------------\n");
