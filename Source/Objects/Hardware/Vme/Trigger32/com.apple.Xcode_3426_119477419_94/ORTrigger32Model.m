...
- (NSData*) getTimeData:(ORDataPacket*)aDataPacket  statusReg:(unsigned short)statusReg eventMask:(uint32_t)aEventMask trigger:(short)trigger;
		...
		if((statusReg & kTrigger1EventMask)){
			...
			if(statusReg & kValidTrigger1GTClockMask){
				...
				if(useSoftwareGtId || useNoHardware){
                    ...
                    event1gtData = [NSData dataWithBytes:&data length:len*sizeof(int32_t)];
		...
		if(statusReg & kTrigger2EventMask){
			...
			if(statusReg & kValidTrigger2GTClockMask){
				...
				if(dataWasTaken){
                    ...
                    event2gtData = [NSData dataWithBytes:&data length:len*sizeof(int32_t)];
				}
			}
...
- (NSData*) getTimeData:(ORDataPacket*)aDataPacket statusReg:(unsigned short)statusReg eventMask:(uint32_t)eventMask trigger: (short)trigger
{
    uint32_t data[3];
    ...
    return [[[NSMutableData allocWithZone:NULL] initWithBytes:data length:3*sizeof(int32_t)] autorelease];
}
...
{      
	...
	NS_DURING
		...
		uint32_t gtid = [self requestGTID];
