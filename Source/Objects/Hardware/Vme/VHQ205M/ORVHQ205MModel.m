...
static uint32_t register_offsets[kNumberOfVHQ205MSRegisters] = {
...
NSString* ORVHQ205MModelPollingErrorChanged = @"ORVHQ205MModelPollingErrorChanged";
NSString* ORVHQ205MModelStatusReg1Changed	= @"ORVHQ205MModelStatusReg2Changed";
NSString* ORVHQ205MModelStatusReg2Changed	= @"ORVHQ205MModelStatusReg2Changed";
NSString* ORVHQ205MSettingsLock				= @"ORVHQ205MSettingsLock";
NSString* ORVHQ205MSetVoltageChanged		= @"ORVHQ205MSetVoltageChanged";
NSString* ORVHQ205MActVoltageChanged		= @"ORVHQ205MActVoltageChanged";
NSString* ORVHQ205MRampRateChanged			= @"ORVHQ205MRampRateChanged";
NSString* ORVHQ205MPollTimeChanged			= @"ORVHQ205MPollTimeChanged";
NSString* ORVHQ205MModelTimeOutErrorChanged	= @"ORVHQ205MModelTimeOutErrorChanged";
NSString* ORVHQ205MActCurrentChanged		= @"ORVHQ205MActCurrentChanged";
NSString* ORVHQ205MMaxCurrentChanged		= @"ORVHQ205MMaxCurrentChanged";

@implementation ORVHQ205MModel
...
- (void) setUpImage
{
    [self setImage:[NSImage imageNamed:@"VHQ205M"]];
}

- (void) makeMainController
{
    [self linkToController:@"ORVHQ205MController"];
}

- (NSString*) helpURL
{
	return @"VME/VHQ205M.html";
}
...
- (void) setPollingError:(BOOL)aPollingError
{
	if(pollingError!= aPollingError){
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MModelPollingErrorChanged object:self];
	}
}

- (unsigned short) statusReg1Chan:(unsigned short)aChan
{
	if(aChan>=kNumVHQ205MChannels)return 0;
    return statusReg1Chan[aChan];
}

- (void) setStatusReg1Chan:(unsigned short)aChan withValue:(unsigned short)aStatusWord
{
	if(aChan>=kNumVHQ205MChannels)return;
...
- (void) setStatusReg2Chan:(unsigned short)aChan withValue:(unsigned short)aStatusWord
{
	...
	if(statusReg2Chan[aChan] != aStatusWord){
		...
		NSDictionary* userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:aChan] forKey:@"Channel"];
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MModelStatusReg2Changed object:self userInfo:userInfo];
	}
}
...
- (void) setTimeErrorState:(BOOL)aState
{
	if(timeOutError != aState){
		timeOutError = aState;
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MModelTimeOutErrorChanged object:self];
	}
}
...
- (void) setPollTime:(int)aPollTime
{
    ...
    pollTime = aPollTime;
	[self pollHardware];
    [[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MPollTimeChanged object:self];
}
...
- (void) setVoltage:(unsigned short) aChan withValue:(float) aVoltage
{
    ...
    if(aVoltage >= 0 && aVoltage <= kVHQ205MMaxVoltage) voltage[aChan] = aVoltage;
    if(aVoltage < 0) NSLogColor([NSColor redColor], @"ORVHQ205MModel: voltage must be > 0, polarity is set in hardware\n");
    else if(aVoltage >= kVHQ205MMaxVoltage) NSLogColor([NSColor redColor], @"ORVHQ205MModel: maximum allowed voltage is 4000 V\n");
    [[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MSetVoltageChanged object:self userInfo: nil];
}
...
- (void) setActVoltage:(unsigned short) aChan withValue:(float) aVoltage
{
	...
	if(actVoltage[aChan] != aVoltage){
		...
		NSDictionary* userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:aChan] forKey:@"Channel"];
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MActVoltageChanged object:self userInfo: userInfo];
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MModelStatusReg1Changed object:self userInfo: userInfo]; //also send this to force some updates
	}
}
...
- (void) setActCurrent:(unsigned short) aChan withValue:(float) aCurrent
{
	...
	if(actCurrent[aChan] != aCurrent){
		...
		NSDictionary* userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:aChan] forKey:@"Channel"];
		[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MActCurrentChanged object:self userInfo: userInfo];
	}
}
...
- (void) setMaxCurrent:(unsigned short) aChan withValue:(float) aCurrent
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MMaxCurrentChanged object:self userInfo: nil];
}

- (unsigned short) rampRate:(unsigned short) aChan
{
	if(aChan>=kNumVHQ205MChannels)return 2;
...
- (void) setRampRate:(unsigned short) aChan withValue:(unsigned short) aRampRate
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:ORVHQ205MRampRateChanged object:self userInfo: nil];
}
...
- (void) pollHardware
{
	...
	@catch(NSException* e){
		...
		NSLogError(@"",@"VHQ205M",@"Polling Error",nil);
	}
...
- (void) loadValues:(unsigned short)aChannel
{
	...
	if(aChannel>=kNumVHQ205MChannels)return;
...
- (void) readModuleID
{
	...
	unsigned short serialNumber =	(aValue>>12)*1000 + 
									...
									((aValue&0x00f0)>>4) *10 + 
									(aValue &0x000f);
	NSLog(@"VHQ205M (Slot %d) Serial Number = %d\n", [self slot], serialNumber);
}
...
- (eVHQ205MRampingState) rampingState:(unsigned short)aChannel
{
	if(aChannel>=kNumVHQ205MChannels)return kHVOff;
	if(!(statusReg1Chan[aChannel] & kHVSwitch)){
		if(statusReg1Chan[aChannel] & kStatV) {
...
{
	if(aChannel>=kNumVHQ205MChannels)return 0;
...
{
	if(aChannel>=kNumVHQ205MChannels)return NO;
...
- (void) appendDataDescription:(ORDataPacket*)aDataPacket userInfo:(NSDictionary*)userInfo
{
    ...
    [aDataPacket addDataDescriptionItem:[self dataRecordDescription] forKey:@"VHQ205MModel"];
}
...
- (NSDictionary*) dataRecordDescription
{
    ...
    NSDictionary* aDictionary = [NSDictionary dictionaryWithObjectsAndKeys:
								 @"ORVHQ205MDecoderForHVStatus",                 @"decoder",
...
- (id)initWithCoder:(NSCoder*)decoder
{
    ...
    [[self undoManager] disableUndoRegistration];
	int i;	
	for(i=0;i<kNumVHQ205MChannels;i++){
...
- (void) syncDataIdsWith:(id)anotherVHQ205M
{
    [self setDataId:[anotherVHQ205M dataId]];
}
