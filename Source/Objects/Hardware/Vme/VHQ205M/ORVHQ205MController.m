...
#import "ORVHQ205MController.h"
#import "ORVHQ205MModel.h"

@interface ORVHQ205MController (private)
...
@implementation ORVHQ205MController

-(id)init
{
    self = [super initWithWindowNibName:@"VHQ205M"];
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
					 selector : @selector(settingsLockChanged:)
						 name : ORVHQ205MSettingsLock
						object: nil];
	
	[notifyCenter addObserver : self
					 selector : @selector(setVoltageChanged:)
						 name : ORVHQ205MSetVoltageChanged
						object: model];
	
	[notifyCenter addObserver : self
					 selector : @selector(actVoltageChanged:)
						 name : ORVHQ205MActVoltageChanged
						object: model];
		
	[notifyCenter addObserver : self
					 selector : @selector(rampRateChanged:)
						 name : ORVHQ205MRampRateChanged
						object: model];
	
	[notifyCenter addObserver : self
					 selector : @selector(pollTimeChanged:)
						 name : ORVHQ205MPollTimeChanged
						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(statusReg1Changed:)
                         name : ORVHQ205MModelStatusReg1Changed
						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(statusReg2Changed:)
                         name : ORVHQ205MModelStatusReg2Changed
						object: model];
	
//    [notifyCenter addObserver : self
//                     selector : @selector(timeOutErrorChanged:)
//                         name : ORVHQ205MModelTimeOutErrorChanged
//						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(actCurrentChanged:)
                         name : ORVHQ205MActCurrentChanged
						object: model];	
	
    [notifyCenter addObserver : self
                     selector : @selector(maxCurrentChanged:)
                         name : ORVHQ205MMaxCurrentChanged
						object: model];	
	
    [notifyCenter addObserver : self
                     selector : @selector(pollingErrorChanged:)
                         name : ORVHQ205MModelPollingErrorChanged
						object: model];
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORVHQ205MSettingsLock to:secure];
...
- (void) settingsLockChanged:(NSNotification*)aNotification
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORVHQ205MSettingsLock];
    BOOL locked = [gSecurity isLocked:ORVHQ205MSettingsLock];
    ...
    if(lockedOrRunningMaintenance){
		if(runInProgress && ![gSecurity isLocked:ORVHQ205MSettingsLock])s = @"Not in Maintenance Run.";
    }
...
- (void) statusReg1Changed:(NSNotification*)aNote
{
	...
	eVHQ205MRampingState state = [model rampingState:chan];
	switch (state) {
...
- (void) slotChanged:(NSNotification*)aNotification
{
	...
	[[self window] setTitle:[NSString stringWithFormat:@"VHQ205M Card (Slot %d)",[model slot]]];
}
...
{
    [gSecurity tryToSetLock:ORVHQ205MSettingsLock to:[sender intValue] forWindow:[self window]];
}
...
- (IBAction) readModuleID:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Module ID Read of VHQ205M FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed Module ID Read of VHQ205M", @"OK", nil, nil,
                        localException);
    }
}
...
- (IBAction) loadAllValues:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Hardware access of VHQ205M FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed HW Access of VHQ205M", @"OK", nil, nil,
                        localException);
    }
}
...
- (IBAction) readStatus:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Status Read of VHQ205M FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed Status Read of VHQ205M", @"OK", nil, nil,
                        localException);
    }
}
@end
					  
@implementation ORVHQ205MController (private)
- (void) panicToZero:(unsigned short)aChannel
{
	[self endEditing];
#if defined(MAC_OS_X_VERSION_10_10) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_10 // 10.10-specific
    ...
    [alert beginSheetModalForWindow:[self window] completionHandler:^(NSModalResponse result){
        if (result == NSAlertFirstButtonReturn){
            ...
            @catch(NSException* e){
                NSLog(@"vhW205M Panic failed because of exception\n");
            }
        }
    }];
