...
#import "ORVHS4060nController.h"
#import "ORVHS4060nModel.h"

@interface ORVHS4060nController (private)
...
@implementation ORVHS4060nController

-(id)init
{
    self = [super initWithWindowNibName:@"VHS4060n"];
...
- (void) slotChanged:(NSNotification*)aNotification
{
	...
	[[self window] setTitle:[NSString stringWithFormat:@"VHS4060n Card (Slot %d)",[model slot]]];
}
...
- (void) voltageRampSpeedChanged:(NSNotification*)aNote	{
    [voltageRampSpeedField setFloatValue: [model voltageRampSpeed]];
//    NSLogFont(f,@"Ramp Speed   : %.1f%% = %.1f V/s\n",[self readModuleRegIndex:kVoltageRampSpeed],);
    [voltageRampSpeedTranslatedField setFloatValue:6000*[model voltageRampSpeed]/100.];
}
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORVHS4060nSettingsLock to:secure];
...
- (void) settingsLockChanged:(NSNotification*)aNotification
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORVHS4060nSettingsLock];
    BOOL locked = [gSecurity isLocked:ORVHS4060nSettingsLock];
    ...
    if(lockedOrRunningMaintenance){
		if(runInProgress && ![gSecurity isLocked:ORVHS4060nSettingsLock])s = @"Not in Maintenance Run.";
    }
...
- (void) voltageSetChanged:(NSNotification*)aNote
{
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[voltageSetMatrix cellAtRow:i column:0] setFloatValue:[model voltageSet:i]];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
- (void) voltageMeasureChanged:(NSNotification*)aNote
{
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[voltageMeasureMatrix cellAtRow:i column:0] setFloatValue:[model voltageMeasure:i]];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
- (void) currentMeasureChanged:(NSNotification*)aNote
{
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[currentMeasureMatrix cellAtRow:i column:0] setFloatValue:[model currentMeasure:i]*1000.];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
{	
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[currentSetMatrix cellAtRow:i column:0] setFloatValue:[model currentSet:i]*1000.];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
- (void) voltageBoundsChanged:(NSNotification*)aNote
{
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[voltageBoundsMatrix cellAtRow:i column:0] setFloatValue:[model voltageBounds:i]];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
- (void) currentBoundsChanged:(NSNotification*)aNote
{
	if(!aNote){
		int i; 
		for(i=0;i<kNumVHS4060nChannels;i++) [[currentBoundsMatrix cellAtRow:i column:0] setFloatValue:[model currentBounds:i]*1000.];	
	}
	else {
		int chan = [[[aNote userInfo] objectForKey:@"Channel"] intValue];	
...
- (IBAction) toggleHVOnOffAction:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"HV power toggle of VHS4060n FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed HV power toggle of VHS4060n", @"OK", nil, nil,
                        localException);
    }
...
- (IBAction) reportAction:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Module Info Read of VHS4060n FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed Module Info Read of VHS4060n", @"OK", nil, nil,
                        localException);
    }
}
...
- (IBAction) doClearAction:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Do Clear of VHS4060n FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed Do Clear of VHS4060n", @"OK", nil, nil,
                        localException);
    }
}
...
{
    [gSecurity tryToSetLock:ORVHS4060nSettingsLock to:[sender intValue] forWindow:[self window]];
}
...
- (IBAction) loadStartAction:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Hardware access of VHS4060n FAILED.\n");
        ORRunAlertPanel([localException name], @"%@\nFailed HW Access of VHS4060n", @"OK", nil, nil,
                        localException);
    }
}
...
@implementation ORVHS4060nController (private)
- (void) panicToZero:(unsigned short)aChannel
{
	[self endEditing];
