...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(stealthModeChanged:)
                         name : ORInFluxDBModelStealthModeChanged
						object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(useHttpsChanged:)
                         name : ORInFluxDBModeUseHttpsChanged
                        object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(dataBaseInfoChanged:)
                         name : ORInFluxDBModelDBInfoChanged
						object: model];	
	
    [notifyCenter addObserver : self
                     selector : @selector(keepHistoryChanged:)
                         name : ORInFluxDBModelKeepHistoryChanged
						object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(replicationRunningChanged:)
                         name : ORInFluxDBModelReplicationRunningChanged
						object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(usingUpdateHandlerChanged:)
                         name : ORInFluxDBModelUsingUpdateHandleChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(alertMessageChanged:)
                         name : ORInFluxDBModelAlertMessageChanged
                        object: model];

    [notifyCenter addObserver : self
                     selector : @selector(alertTypeChanged:)
                         name : ORInFluxDBModelAlertTypeChanged
                        object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(skipDataSetsChanged:)
                         name : ORInFluxDBModelSkipDataSetsChanged
                        object: model];

    [notifyCenter addObserver : self
                     selector : @selector(postRunningScriptsChanged:)
                         name : ORInFluxDBModelPostRunningScriptsChanged
                       object : model];
}
