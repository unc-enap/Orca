...
NSString* ORInFluxDBModelSkipDataSetsChanged       = @"ORInFluxDBModelSkipDataSetsChanged";
NSString* ORInFluxDBModelAlertTypeChanged          = @"ORInFluxDBModelAlertTypeChanged";
NSString* ORInFluxDBModelAlertMessageChanged       = @"ORInFluxDBModelAlertMessageChanged";
NSString* ORInFluxDBModelReplicationRunningChanged = @"ORInFluxDBModelReplicationRunningChanged";
NSString* ORInFluxDBModelKeepHistoryChanged		  = @"ORInFluxDBModelKeepHistoryChanged";
NSString* ORInFluxDBModelStealthModeChanged		  = @"ORInFluxDBModelStealthModeChanged";
NSString* ORInFluxDBModelPostRunningScriptsChanged = @"ORInFluxDBModelPostRunningScriptsChanged";
...
NSString* ORInFluxDBModelUsingUpdateHandleChanged  = @"ORInFluxDBModelUsingUpdateHandleChanged";
NSString* ORInFluxDBModeUseHttpsChanged            = @"ORInFluxDBModeUseHttpsChanged";
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
					 selector : @selector(addAdcsToHistoryRecord:)
						 name : @"ORInFluxDBAddHistoryAdcRecord"
					   object : nil];

    [notifyCenter addObserver : self
					 selector : @selector(postOrPutCustomRecord:)
						 name : @"ORInFluxDBPostOrPutCustomRecord"
					   object : nil];
...
- (void) setSkipDataSets:(BOOL)aSkipDataSets
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelSkipDataSetsChanged object:self];
}
...
- (void) setAlertType:(int)aAlertType
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelAlertTypeChanged object:self];
}
...
- (void) setAlertMessage:(NSString*)aAlertMessage
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelAlertMessageChanged object:self];
}
...
- (void) setUsingUpdateHandler:(BOOL)aState
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelUsingUpdateHandleChanged object:self];
}
...
- (void) setReplicationRunning:(BOOL)aReplicationRunning
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelReplicationRunningChanged object:self];
}
...
- (void) setKeepHistory:(BOOL)aKeepHistory
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelKeepHistoryChanged object:self];
}
- (BOOL) useHttps
{
    return useHttps;
...
- (void) setUseHttps:(BOOL)aState;
{
    ...
    useHttps = aState;
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModeUseHttpsChanged object:self];
}
...
- (void) setStealthMode:(BOOL)aStealthMode
{
    ...
    if(stealthMode){
        if([ORInFluxDBQueue operationCount]) [ORInFluxDBQueue cancelAllOperations];
        [self _cancelAllPeriodicOperations];
    }
    else {
        [self createDatabases];
    }
	[[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelStealthModeChanged object:self];
}
...
- (void) setPostRunningScripts:(BOOL)postRunning
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelPostRunningScriptsChanged object:self];
}
...
- (ORInFluxDB*) statusDBRef:(NSString*)aDatabaseName
{
    if (localHostName == nil) {
        [self setLocalHostName:@"localhost"];
    }
	return [ORInFluxDB couchHost:localHostName port:portNumber username:userName pwd:password database:aDatabaseName delegate:self];
}
...
- (ORInFluxDB*) historyDBRef:(NSString*)aDatabaseName
{
    if (localHostName == nil) {
        [self setLocalHostName:@"localhost"];
    }
...
- (ORInFluxDB*) remoteHistoryDBRef:(NSString*)aDatabaseName
{
	...
	else return [ORInFluxDB couchHost:remoteHostName port:portNumber username:userName pwd:password database:aDatabaseName delegate:self];
}
...
- (void) setDBInfo:(NSDictionary*)someInfo
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:ORInFluxDBModelDBInfoChanged object:self];
}
	...
	@synchronized(self){
		if([aResult isKindOfClass:[NSDictionary class]]){
			...
			if(message){
                ...
                if([aTag isEqualToString:kCreateDB]){
                    if([[aResult objectForKey:@"Reason"] rangeOfString:@"already exists"].location == NSNotFound){
                        [aResult prettyPrint:@"InFluxDB Message:"];
                    }
                }
				else [aResult prettyPrint:@"InFluxDB Message:"];
			}
			else {
				if([aTag isEqualToString:kInfoDB]){
					[aResult prettyPrint:@"InFluxDB Info:"];
				}
					...
					//[aResult prettyPrint:@"InFluxDB Compacted:"];
				}
                    ...
                    if([[aResult objectForKey:@"error"] isEqualToString:@"conflict"]){
                        ...
                        [aResult prettyPrint:@"InFluxDB Update Handler Installation:"];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self setUsingUpdateHandler:YES];
                        });
                   }
                    else {
                        [aResult prettyPrint:@"InFluxDB Update Handler Installation Error:"];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self setUsingUpdateHandler:NO];
                        });
                    }
			...
			if([aTag isEqualToString:kListDB]){
				[aResult prettyPrint:@"InFluxDB List:"];
			}
			...
			else [aResult prettyPrint:@"InFluxDB"];
		}
		else {
			NSLog(@"%@\n",aResult);
		}
...
- (void) postOrPutCustomRecord:(NSDictionary*)aRecord toAddress:(NSString*)anAddr withDelegate:(id)del
{
    ...
    if(!stealthMode){
        ...
        ORInFluxDB* ref = [self statusDBRef:anAddr];
        [ref setDelegate:del];
...
- (void) updateExperiment
{
	if(!stealthMode){
        ...
        @try {
            id experiment = [self objectConnectedTo:ORInFluxDBModelInConnector];
...
- (void) updateDataSets
{
	if(!stealthMode && !skipDataSets){
		[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(updateDataSets) object:nil];
        @try {
            if([[ORInFluxDBQueue sharedInFluxDBQueue]lowPriorityOperationCount]<10){
